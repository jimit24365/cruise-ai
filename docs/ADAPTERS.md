...
| Derived | sessions, hours, span, prompts, tool calls by type (CLI only), agent name (CLI only), subagent dispatches (CLI parent_session_id chains), MCP tool diversity (CLI tool names include jira, confluence, gitlab, dynacon, etc.), model names (IDE), autonomy mode (IDE), session type (IDE), per-day activity |
