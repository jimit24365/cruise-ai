#!/usr/bin/env node
/**
 * cruise-ai MCP server — your AI coding profile, as tools.
 *
 * Same engine as the CLI: every tool shells out to the cruise-ai
 * Python package or reads its local JSON. Fully local by default; the
 * only tool that sends anything anywhere is cruise_publish, which is
 ...
 *       "cruise-ai": {
 *         "command": "node",
 *         "args": ["/path/to/cruise-ai-mcp/index.js"]
 *       }
 *     }
 *   }
 */
...
// ─── Data directory: $CRUISE_AI_HOME/data or ~/.cruise-ai/data ──────

const USER_HOME = process.env.CRUISE_AI_HOME || join(homedir(), '.cruise-ai');
...
// Self-locating: this file lives at <repo>/cruise-ai-mcp/index.js, so the
...
async function readProfile() {
  try {
    ...
    if (v && v !== EXPECTED_SCHEMA_VERSION) {
      console.error(`[cruise-ai-mcp] Warning: profile schema_version=${v}, expected ${EXPECTED_SCHEMA_VERSION}`);
    }
    return profile;
  } catch (e) {
...
/** Run the cruise-ai CLI: `cruise-ai`, then python3/-m, then python/-m.
...
function runCLI(args = [], timeoutMs = 300_000) {
  const attempts = [
    ['cruise-ai', args, {}],
    ['python3', ['-m', 'cruise_ai', ...args], { cwd: REPO_ROOT }],
    ['python', ['-m', 'cruise_ai', ...args], { cwd: REPO_ROOT }],
  ];
  return new Promise((resolve, reject) => {
    const tryNext = (i) => {
      if (i >= attempts.length) {
        reject(new Error(
          'cruise-ai CLI not found. Tried: `cruise-ai` on PATH, then '
          + `\`python3 -m cruise_ai\` from ${REPO_ROOT}. `
          + 'Run the cruise_doctor tool for a full diagnosis.'
        ));
        return;
      }
      ...
      execFile(cmd, cmdArgs, { timeout: timeoutMs, env: { ...process.env }, ...extra },
        (err, stdout, stderr) => {
          ...
          else reject(new Error(`cruise-ai failed: ${err.message}\n${stderr || stdout || ''}`));
        });
    };
    tryNext(0);
  });
...
const server = new McpServer({ name: "cruise-ai", version: '0.2.0' });

// ── cruise_doctor ───────────────────────────────────────────────────────────────

server.tool(
  'cruise_doctor',
  `Diagnose the cruise-ai installation: is the engine reachable, which Python path works, does a profile exist, what to fix. Run this first if any other tool fails.`,
  {},
  async () => {
    const checks = [];

    const onPath = await probe('cruise-ai', ['--help']);
    checks.push(`[${onPath.ok ? 'ok' : '--'}] \`cruise-ai\` on PATH${onPath.ok ? '' : ' (fine if using the repo clone)'}`);

    const py = await probe('python3', ['-m', 'cruise_ai', '--help'], { cwd: REPO_ROOT });
    checks.push(`[${py.ok ? 'ok' : 'XX'}] \`python3 -m cruise_ai\` from ${REPO_ROOT}${py.ok ? '' : ` — ${py.detail}`}`);

    const pyVer = await probe('python3', ['--version']);
    checks.push(`[${pyVer.ok ? 'ok' : 'XX'}] ${pyVer.ok ? pyVer.detail + ' (3.9+ required)' : 'python3 not found'}`);

    let profileState = 'no profile yet — run cruise_calibrate then cruise_assess';
    try {
      await access(PROFILE_PATH);
      profileState = `profile present at ${PROFILE_PATH}`;
    } catch { /* absent */ }
    checks.push(`[..] ${profileState}`);
    checks.push(`[..] data home: ${USER_HOME} (override with CRUISE_AI_HOME)`);
    checks.push(`[..] node ${process.version} (18+ required)`);

    const healthy = onPath.ok || py.ok;
    let out = `cruise-ai doctor\n${'='.repeat(40)}\n\n${checks.join('\n')}\n\n`;
    out += healthy
      ? 'Engine reachable — all tools should work.'
      : 'Engine NOT reachable. Fix: install Python 3.9+, then either `pip install -e .` from the repo, or keep the repo clone intact (this server runs `python3 -m cruise_ai` from it).';
    return text(out);
  }
);
...
server.tool(
  'cruise_calibrate',
  `Set up cruise-ai data collection: consent per source + collection scope.
Runs non-interactively with maximal defaults (all standard sources, all repos, all-time window). Experimental sources (Claude Desktop) stay OFF unless the user opts in interactively. Everything is read locally; nothing is uploaded.`,
  ...
  async () => {
    try {
      ...
      return text(out.trim() + '\n\nConsent persisted. Run cruise_assess next.');
...
server.tool(
  ...
  async ({ rescan, code, project }) => {
    try {
      ...
      let result = `cruise-ai — Assessment\n${'='.repeat(40)}\n\n` + profileSummary(profile);
...
server.tool(
  'cruise_get_profile',
  `Read the current AI coding profile (the local assessment JSON): dimensions, archetypes, positioning, work mode, wrapped stats, activity. Suggest cruise_assess first if none exists.`,
  {
  ...
  async ({ raw }) => {
    try {
      const profile = await readProfile();
      if (!profile) return text('No profile found. Run cruise_assess first.');
      if (raw) return text(JSON.stringify(profile, null, 2));

      let out = `cruise-ai — Profile\n${'='.repeat(40)}\n\n` + profileSummary(profile);
  ...
  async () => {
    try {
      ...
      if (enr.source === 'heuristic') {
        out += `(heuristic text — run cruise_enrichment_request for an agent-written narrative)\n`;
      }
      out += '\n' + profileSummary(profile);
      ...
      if ((exp.codeIntelligence || []).length) {
        out += `\nCode intelligence: ${exp.codeIntelligence.length} findings (run cruise_get_profile raw=true for detail)\n`;
      }
      out += `\nBrowser view: cruise-ai report → http://localhost:${DEFAULT_PORT}/report`;
      return text(out);
    } catch (e) { return errText(`get_report failed: ${e.message}`); }
  }
);

// ── cruise_enrichment_request / cruise_enrichment_submit ──────────────────────────

server.tool(
  'cruise_enrichment_request',
  `Get the enrichment prompt (ENRICHMENT-PROMPT.md filled with this user's real signals + bounded, secret-stripped excerpts). YOU — the user's own agent — should then produce the six-block JSON it asks for and pass it to cruise_enrichment_submit. Derived-only: never include raw code, prompts, or ranking language. The narrative never changes scores and narrates positioning as ground truth.`,
  {},
  async () => {
    try {
      ...
      return text(
        `Follow the instructions below and produce ONLY the JSON object, then call cruise_enrichment_submit with it.\n\n${prompt}`
      );
...
server.tool(
  'cruise_enrichment_submit',
  `Submit the six-block enrichment JSON produced from cruise_enrichment_request. It is validated on ingest (off-schema keys, raw/fenced code, and ranking language are rejected — fix and resubmit ONCE if rejected). Revocable: cruise-ai enrich --revoke.`,
  {
...
server.tool(
  ...
  {
    out: z.string().optional().describe('Output directory (default: ./cruise-ai-export).'),
  },
  async ({ out }) => {
    try {
      const args = ['export'];
...
server.tool(
  ...
  async ({ port }) => {
    ...
    return text(`Server not running. Start it with:\n  cruise-ai report --port ${p}\nThen visit http://localhost:${p}/profile`);
  }
);

// ── cruise_publish / cruise_unpublish (gated) ──────────────────────────────────────

server.tool(
  'cruise_publish',
  `OPT-IN NETWORK PUBLISH — the only tool that sends data anywhere.
Publishes the user's curated, visibility-filtered, derived-only profile to a registry so hiring managers' agents can discover them. Never sends raw code, transcripts, prompts, hidden projects, private growth, or experimental signals. Revocable via cruise_unpublish.
...
Never call with confirm=true without the user's explicit confirmation in this conversation.`,
  {
    ...
    registry: z.string().optional().describe(`Registry URL (default ${DEFAULT_REGISTRY} — self-hosted; the hosted cruise-ai network is roadmap).`),
  },
  async ({ confirm, registry }) => {
    try {
      ...
      if (!confirm) {
        args.push('--dry-run');
        const out = await runCLI(args);
        return text(out.trim() + '\n\nNothing was sent. Show the user the sections above and ask for explicit confirmation, then call cruise_publish again with confirm=true.');
      }
      args.push('--confirm');
      const out = await runCLI(args);
      return text(out.trim());
...
// ── cruise_discover_builders (the hiring-manager agent side) ───────────────────

server.tool(
  'cruise_discover_builders',
  `Query a cruise-ai registry for builders who opted in to be discovered. Returns only what each builder explicitly published. Filters: leverage mode (prompting | harnessing | designs_the_loop), build domain (products | ai_products | ai_systems), tech domain (e.g. python). Positioning is a map, not a ranking — results are matches, not a leaderboard.`,
  {
...
server.tool(
  ...
  async () => {
    try {
      ...
      let out = `cruise-ai — Growth Edge (private)\n${'='.repeat(45)}\n\n`;
...
server.tool(
  ...
  async ({ job_description }) => {
    try {
      ...
      let out = `cruise-ai — Role Comparison\n${'='.repeat(45)}\n\n`;
      ...
      if (gaps.length) {
        ...
        out += '\nUse cruise_growth_edge for next signals to build.\n';
      }
      return text(out);
