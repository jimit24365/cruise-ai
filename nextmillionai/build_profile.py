#!/usr/bin/env python3
"""
nextmillionai — Build your AI coding profile.
...
Subcommands:
    nextmillionai calibrate    # Onboarding: consent + collection scope
    nextmillionai assess       # Scan + score + write assessment JSON
    nextmillionai report       # Start local profile server
    nextmillionai sources      # Show discovered data sources
    nextmillionai enrich       # (Step 5) AI narrative enrichment

Legacy flags still work:
    nextmillionai --serve      # assess + report
    nextmillionai --preview    # Show collected scan data
    nextmillionai --tools      # List detected AI tools
    nextmillionai --yes        # Non-interactive (accept all sources)
"""
...
from nextmillionai import __version__
...
def run_scan(project_filter=None, enabled_sources=None, collection_config=None, code_scan=False):
    ...
    *code_scan*: opt-in local code scan (``assess --code``) — reads repo
    ...
    ``signal_matrix``.
    """
    from nextmillionai.adapters._registry import get_session_adapters, run_adapters
    ...
    from nextmillionai.scanner import build_summary, compute_normalized, iso_now
    ...
    # The ledger is append-only and local-only (~/.nextmillionai/data/history)
    ...
    if code_scan:
        from nextmillionai.code_intel import scan_repos
    ...
    from nextmillionai import __version__ as _pkg_version
    from nextmillionai.schema import METHODOLOGY_VERSION, TAXONOMY_VERSION
...
def show_preview():
    ...
    if not sr.is_file():
        print("  No scan data yet. Run `nextmillionai assess` first.")
        return
...
def cmd_calibrate(args) -> None:
    ...
    from nextmillionai.paths import collection_config_path, consent_path
...
def _refresh_config_signals(scan_results: dict, enabled_sources: dict) -> None:
    ...
    """
    from nextmillionai.scanner import HOME, count_mcp_servers
    ...
    if desktop_data:
        from nextmillionai.adapters.claude_desktop import default_desktop_dir
...
def cmd_assess(args, *, _show_intro: bool = True) -> None:
    ...
    from nextmillionai.paths import data_dir as _data_dir
    from nextmillionai.paths import profile_path as _profile_path
    from nextmillionai.paths import scan_results_path as _scan_results_path
    ...
    if scan_results is None:
        from nextmillionai.consent import load_collection_config
    ...
    from nextmillionai.aggregator import build_summary_line
    from nextmillionai.profile_data import _build_default_profile, save_profile
    from nextmillionai.schema import SCHEMA_VERSION
    ...
    from nextmillionai.scanner import iso_now as _iso_now
    ...
    from nextmillionai.schema import METHODOLOGY_VERSION as _METHODOLOGY_VERSION
    ...
    from nextmillionai.enrichment import build_heuristic_enrichment
    ...
    from nextmillionai.history import append_snapshot
    ...
    from nextmillionai.sync_merge import apply_multi_device
...
def cmd_report(args) -> None:
    ...
    if demo:
        from nextmillionai.paths import PACKAGE_DIR

        example = PACKAGE_DIR / "examples" / "profile.json"
        os.environ["NEXTMILLIONAI_PROFILE_PATH"] = str(example)
    ...
    from nextmillionai import cliui as _rui
    ...
    if live:
        from nextmillionai.live import discover_watch_paths
    ...
    open_browser = (
        ...
        and not os.environ.get("NEXTMILLIONAI_NO_BROWSER")
...
def cmd_enrich(args) -> None:
    ...
    from nextmillionai.paths import profile_path, scan_results_path
    ...
    if getattr(args, "revoke", False):
        from nextmillionai.enrichment import revoke_enrichment
    ...
    if submit_file:
        from nextmillionai.enrichment import parse_submission
    ...
    from nextmillionai.enrichment import build_evidence_bank, prompt_file_text
...
def cmd_export(args) -> None:
    ...
    out = getattr(args, "out", "./nextmillionai-export")
    print()
    print("  nextmillionai — Export")
    print("  " + "=" * 42)
    print()
    try:
...
def cmd_publish(args) -> None:
    ...
    print()
    print("  nextmillionai — Publish (opt-in)")
    print("  " + "=" * 42)
    print()
    ...
    if not getattr(args, "dry_run", False) and not registry_reachable(registry):
        ...
        print()
        print("  No hosted nextmillionai registry exists yet. Either:")
    ...
    print("  Revocable anytime: nextmillionai unpublish")
    print()
...
def cmd_sync(args) -> None:
    ...
    from nextmillionai import cliui as _cliui
    ...
    if getattr(args, "status", False):
        from nextmillionai.sync_merge import device_identity, load_device_snapshots
    ...
    if not cfg and not repo_url:
        ...
        print(f"    {cli_invocation()} sync --repo git@github.com:YOU/nma-sync.git")
        print()
        return
    ...
    if not cfg and repo_url:
        ...
        print("  Revocable anytime: nextmillionai sync --revoke")
        print()
...
def cmd_network(args) -> None:
    ...
    if action == "serve":
        ...
        from nextmillionai.network_server import run_registry
    ...
    # status (default)
    from nextmillionai.network import load_publish_state
...
def cmd_sources(args) -> None:
    ...
    print()
    print("  nextmillionai — Discovered Sources")
    print("  " + "=" * 42)
    print()
...
def main():
    parser = argparse.ArgumentParser(
        prog="nextmillionai",
        description="nextmillionai — Build your AI coding profile",
        epilog=(
            "the pipeline:  calibrate -> assess -> report -> enrich -> sync/publish\n"
            "run `nextmillionai guide` for the visual map of every step\n"
    ...
    export.add_argument(
        "--out",
        type=str,
        default="./nextmillionai-export",
        help="Output directory (default: ./nextmillionai-export)",
    )
    ...
    network.add_argument(
        "--store", type=str, help="Registry store directory (default: ~/.nextmillionai/registry)"
    )
    ...
    parser.add_argument("--version", action="version", version=f"nextmillionai {__version__}")
    ...
    if getattr(args, "verbose", False):
        os.environ["NEXTMILLIONAI_VERBOSE"] = "1"
...
def _handle_legacy(args) -> None:
    ...
    if getattr(args, "tools", False):
        from nextmillionai.scanner import list_tools
    ...
    if getattr(args, "reset_consent", False):
        from nextmillionai.consent import reset_consent
