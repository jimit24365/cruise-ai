"""
nextmillionai -- Profile data module.
Loads/saves developer profile from data/profile.json.
Identity fields (name, title, location, etc.) come from nextmillionai.config.json.
"""
...
from nextmillionai.paths import config_path, profile_path
...
def _load_identity():
    """Load identity fields from nextmillionai.config.json if present."""
...
def load_profile():
    """Load profile from ~/.nextmillionai/data/profile.json if exists, else return default.

    ``NEXTMILLIONAI_PROFILE_PATH`` overrides the location — used by
    `report --demo` to serve the bundled example without touching the
    user's real data."""
    import os as _os

    override = _os.environ.get("NEXTMILLIONAI_PROFILE_PATH")
    if override:
        try:
...
def build_agent_profile(profile, port=7749, visibility=None):
    ...
    return {
        ...
        "additionalType": "nextmillionai:AIEngineerProfile",
        "nextmillionai": {
