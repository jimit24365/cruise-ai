"""
nextmillionai.network -- Opt-in network publish/unpublish client.
...
runs `nextmillionai publish` and explicitly confirms. What is sent is
...
revocable: `nextmillionai unpublish` deletes the record.
"""
...
from nextmillionai.visibility import load_visibility_config

# No hosted nextmillionai registry exists yet (roadmap). Default points
# at a self-hosted reference registry (`nextmillionai network serve`).
...
_SYNC_README = """# nextmillionai sync store

Derived-only multi-device snapshots written by `nextmillionai sync`.
...
Remove a device with `nextmillionai sync --revoke` on that device.
"""
...
def sync_run(repo_url: str | None = None) -> dict:
    ...
    from nextmillionai.sync_merge import device_identity, devices_dir, write_own_snapshot

    cfg = load_sync_config()
    if repo_url:
        from nextmillionai.scanner import iso_now

        cfg = {"repoUrl": repo_url, "configuredAt": iso_now()}
        sync_config_path().write_text(json.dumps(cfg, indent=2))
    if not cfg:
        raise RuntimeError(
            "No sync repo configured. Run `nextmillionai sync --repo "
...
def sync_revoke() -> str:
    ...
    from nextmillionai.sync_merge import device_identity, devices_dir
