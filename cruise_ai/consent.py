"""
cruise_ai.consent -- First-run consent and data-collection disclosure.

Manages user consent for scanning each data source. Consent state
is persisted to ``~/.cruise_ai/data/consent.json``.

Collection scope (day-window, repos filter) is persisted to
``~/.cruise_ai/data/collection_config.json``.
"""
...
from cruise_ai.paths import collection_config_path, data_dir
...
def print_disclosure(only: list[str] | None = None) -> None:
    ...
    *only*: restrict the per-source blocks to these ALL_SOURCES keys
    ...
    print("  " + "=" * 42)
    print()
    print("  cruise_ai scans local AI session data")
