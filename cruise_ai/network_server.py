"""
cruise_ai.network_server -- Reference registry for the cruise_ai
network (`cruise_ai network serve`).

A minimal, self-hostable, file-backed registry. This is the SAME code a
hosted cruise_ai.org registry would run — hosting it is roadmap; the
...
from cruise_ai.schema import SHAREABLE_PROFILE_FIELDS
...
def make_handler(store: RegistryStore):
    class RegistryHandler(BaseHTTPRequestHandler):
        server_version = "cruise_ai-registry/0.1"
...
def run_registry(port: int = 7750, host: str = "127.0.0.1", store_dir: Path | None = None):
    ...
    print(f"  cruise_ai registry on http://{host}:{port}")
    print(f"  store: {store.dir}")
    print("  Builders appear here only after an explicit `cruise_ai publish`.")
    try:
