...
var NMA_PROV_SCAFFOLD=`
    <section>
      ...
      <div class="ctl-note">Scores come only from your local AI coding sessions + git over the selected window. Widen the window or add repos with <b>cruise_ai calibrate</b> &mdash; nothing leaves your machine.</div>
...
function renderProvenance(P){
  ...
  }).join(''):'<div class="ctl-note">No links added. Add them in <b>cruise_ai.config.json</b> — shown as self-declared, never scored.</div>';
}
...
function renderShare(){
  ...
  host.innerHTML=
    ...
    +'<div class="share-card">'
      ...
      +'<div class="sc-d">Any MCP-compatible agent (Claude Code, Cursor, Cline) will be able to read this profile and run assessments via the cruise_ai MCP server.</div>'
    +'</div>'
...
function downloadCard(){
  ...
  x.font='500 11px "JetBrains Mono", monospace';x.fillStyle='#8C877A';
  x.fillText('cruise_ai.org',36,332);
...
function cliCmd(sub){return (window._CLI||'cruise_ai')+' '+sub;}
