...
Canonical source: [`cruise_ai/schema.py`](../schema.py)
...
|----------|------|-------------|-----------|
| **ScanResult** | `~/.cruise_ai/data/scan_results.json` | LOCAL ONLY | none |
| **Profile** | `~/.cruise_ai/data/profile.json` | Full on disk; shareable via builder | `GET /api/profile` (full, localhost), `GET /api/profile/public` (stripped) |
...
Both views — the profile and report pages in `cruise_ai/static/` (the
report) — render from this **same** assessment object. One source of truth, no
parallel shapes. The object lives on disk at `~/.cruise_ai/data/profile.json`
