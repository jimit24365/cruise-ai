# cruise_ai — Scoring Methodology

How cruise_ai measures how you build with AI. Every score, archetype, work
...
· Formula fingerprint: `47d7286a2f37` *(version-stable source hash of the scoring
...
1. **Your code and data never reach cruise_ai.** We have no server and no
...
cruise_ai reports you in three distinct layers — keep them separate:
...
model is the user's own agent (via MCP or `cruise_ai enrich`), never a server of
...
| Custom adapters *(user-registered)* | counts | paths you declare in `cruise_ai.config.json` | file-per-session timestamps or presence |
...
from the same machine-readable registry (`cruise_ai/methodology_spec.py`) that
...
- `0.3.0` — Added the **positioning layer** (build domain × leverage mode + tech
  ...
  **two-promise privacy model** and the links/remote nuance. Added the **assessment
  ...
  no percentiles.
- `0.2.0` — De-branded to cruise_ai; removed traditional-engineer bias
