#!/usr/bin/env python3
"""
cruise_ai — Local profile server.
...
from cruise_ai.paths import DOCS_DIR, PACKAGE_DIR, STATIC_DIR
from cruise_ai.profile_data import (
    build_agent_profile,
    build_profile_meta,
    load_profile,
)
...
class ProfileHandler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/" or path == "":
            ...
            from cruise_ai.methodology_spec import build_spec
            ...
            if sr.is_file():
                ...
                self.send_json({"error": "No scan data. Run cruise_ai first."}, status=404)
...
def run_server(port=None, live=False, open_browser=False):
    ...
    live_line = ""
    # if live:
    #     from cruise_ai.live import start_watcher
    #
    #     watcher = start_watcher()
    #     live_line = "\n    Live:     watching local sources — views update in place\n"

    print(f"""
    cruise_ai -- Profile Server
{live_line}
