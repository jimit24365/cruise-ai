"""
cruise_ai.enrichment -- Enrichment module.
...
Run paths:
  - copy/paste (default): `cruise_ai enrich` prints prompt+excerpts
    to stdout and saves to a file. User runs in their own agent, then
    `cruise_ai enrich --submit result.json` ingests the result.
...
cruise_ai/enrichment.py (excerpt selection + evidence bank).
